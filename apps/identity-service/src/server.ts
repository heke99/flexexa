...
import {DomainError,record,tenantId} from '@flexexa/domain';
...
export function createIdentityServer(config:Connection & {publishableKey:string;adminKey:string;environment:string}){
 const environment=connectEnvironment(config.environment),admin=createReservedAuthAdmin(config,config.adminKey);
 let active=0;
 const server=createServer(async(req,res)=>{
  ...
  try{
   ...
   const rpc=createCallerRpc(config,config.publishableKey,match[1]!);
