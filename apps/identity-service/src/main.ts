...
try{
 ...
 const environment=required('FLEXEXA_ENVIRONMENT');
 const telemetry=startTelemetry(process.env.OTEL_EXPORTER_OTLP_TRACES_ENDPOINT,environment,Number(process.env.FLEXEXA_TRACE_SAMPLE_RATIO??0.1));
