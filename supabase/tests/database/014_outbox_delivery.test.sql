...
insert into public.api_clients(id,tenant_id,client_id,name,expires_at) values(pg_temp.oid(60),pg_temp.oid(20),'outbox-api','Outbox API',now()+interval '1 hour');
insert into private.flexexa_machine_principals(id,auth_user_id,principal_type,tenant_id,api_client_id,environment)
 values(pg_temp.oid(51),pg_temp.oid(2),'api_client',pg_temp.oid(20),pg_temp.oid(60),'sandbox');
insert into public.api_client_permissions(tenant_id,api_client_id,permission_id,condition_json)
 select pg_temp.oid(20),pg_temp.oid(60),id,'{"environment":"sandbox"}' from public.permissions where permission_key='events.publish';
...
select is((select count(*) from pg_proc where oid in('public.flexexa_claim_outbox_event(uuid,text)'::regprocedure,'public.flexexa_finish_outbox_event(uuid,text,uuid,text)'::regprocedure) and not prosecdef),2::bigint,'public entrypoints are invokers');
select is((select count(*) from public.role_template_permissions r join public.permissions p on p.id=r.permission_id where p.permission_key='events.publish'),0::bigint,'new permission grants no role template access');
set local role authenticated;
select pg_temp.claims(2,102);
select throws_ok($$select pg_temp.claim_event()$$,'42501','PERMISSION_DENIED','API client denied even with matching tenant/environment grant');
select pg_temp.claims(3,101);
select throws_ok($$select pg_temp.claim_event()$$,'42501','PERMISSION_DENIED','unbound human denied');
...
insert into private.flexexa_outbox_leases(id,tenant_id,event_id,generation,principal_id,session_id,environment,created_at,expires_at)
 values(pg_temp.oid(800),pg_temp.oid(20),pg_temp.oid(204),1,pg_temp.oid(50),pg_temp.oid(101),'sandbox',statement_timestamp()-interval '1 minute',statement_timestamp()-interval '30 seconds');
...
-- Isolate later assertions from scheduler delays exceeding this retry's backoff.
update public.outbox_events set available_at=clock_timestamp()+interval '1 hour' where id=pg_temp.oid(204);
...
select throws_ok($$insert into private.flexexa_outbox_leases(tenant_id,event_id,generation,principal_id,session_id,environment,created_at,expires_at)
 select pg_temp.oid(21),event_id,3,principal_id,session_id,environment,created_at,expires_at from private.flexexa_outbox_leases where id=pg_temp.oid(800)$$,'23503',null,'lease composite FK rejects another tenant event');
select throws_ok($$insert into private.flexexa_outbox_delivery_results(tenant_id,lease_id,requested_outcome,status)
 values(pg_temp.oid(21),pg_temp.oid(800),'retry','pending')$$,'23503',null,'result composite FK rejects another tenant lease');
